...
describe('mongoLocker', () => {
    ...
    it('should acquire and release a lock', async () => {
        ...
        const result1 = await mongoLocker('AAA', callback);
        assert.strictEqual(result1, 1);

        const result2 = await mongoLocker('AAA', callback);
    ...
    it('should not be possible to acquire the lock twice at once', async () => {
        ...
        const callLocker = () => mongoLocker('AAA', callback, { noLaterThan: 0 });
    ...
    it('should propagate an error and release the lock if the callback fails', async () => {
        ...
        await assert.rejects(() => mongoLocker('AAA', async () => {
            throw new Error('Some error occurred');
        }), /Some error occurred/);

        const callback = sinon.spy();
        await mongoLocker('AAA', callback);
    ...
    it('should be able to handle few concurrent tasks by waiting some time and acquire retries', async () => {
        ...
        const callLocker = () => mongoLocker('AAA', callback);
