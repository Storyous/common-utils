...
const findRecordOrThrow = async (collection, key, projection) => {
    const record = findRecord(collection, key, projection);
    if (!record) {
    ...
    const refreshRecord = once(async () => {
        ...
        const refreshPromise = (async () => {
            ...
            try {
                ...
                const finalContent = await cacheConfig.transform(newContent);
        ...
        const refreshResult = await new Promise((resolve, reject) => {
            const timeoutId = setTimeout(() => {
                ...
                findRecordOrThrow(cacheConfig.collection, key).then(resolve, reject);
            }, cacheConfig.timeout);

            refreshPromise.then(resolve, reject).finally(() => {
